...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check_ic = "SELECT * FROM person WHERE person_IC = '$ic'";
    $result = $conn->query($sql_check_ic);

    if ($result->num_rows > 0) {
        echo "<script>alert('Error: IC already exists in the system. Please enter a different IC.'); history.back();</script>";
        exit();
    }    

    $sql_staff = "INSERT INTO staff (staff_department, staff_position, staff_basicsalary, staff_hireddate, staff_status) 
    ...
    if ($conn->query($sql_staff) === TRUE) {
        ...
        if ($conn->query($sql_person) === TRUE) {
            ...
            $usr_pwd = $staff_ID . 'pwd' . $random_digits;

            $sql_usr = "INSERT INTO usr (usr_ID, staff_ID, usr_pwd) 
                        VALUES ('$usr_ID', '$staff_ID', '$usr_pwd')";

            if ($conn->query($sql_usr) === TRUE) {
